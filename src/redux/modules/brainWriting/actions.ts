...
export const getNickname = createAsyncThunk(
  ...
  export const timerData = createAsyncThunk(
    `${prefix}/GET_TIMER_DATA`,async(bwRoomId: string| null )=>{
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/brainwriting/timer/${bwRoomId}`);
      console.log(response.data, "남은타이머 응답데이터")
      return response;
